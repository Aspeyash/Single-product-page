 ...
 *
 * This plugin therefore NEVER declares a shared global name, and never uses
 * the zymarg_sp_ prefix either -- that family belongs to the separate ZYMARG
 * Store Page plugin (see the header comment in zymarg-single-product.php for
 * the full history of that collision). This file declares one plugin-prefixed
 * wrapper, zymarg_sngl_spark(), that delegates, in order, to:
 *
 *   1. zymarg_discovery_spark()  -- the theme, canonical owner of the mark
 *   2. zymarg_vd_spark()         -- the Vendor Dashboard plugin's wrapper
 *   3. zymarg_sp_spark()         -- the Store Page plugin's wrapper, if that
 *                                   happens to be the only other Spark source
 *                                   active on the site
 *   4. a local fallback copy     -- standalone use, or theme switched
 *
 * Do not "simplify" this wrapper back to a shared name.
 *
...
if ( ! function_exists( 'zymarg_sngl_register_shared_brand_assets' ) ) {
	/**
	 ...
	 * @return void
	 */
	function zymarg_sngl_register_shared_brand_assets() {
		...
		if ( ! wp_style_is( 'zymarg-spark', 'registered' ) ) {
			wp_register_style(
				'zymarg-spark',
				ZYMARG_SNGL_ASSETS . 'css/zymarg-spark.css',
				array( 'zymarg-tokens' ),
				ZYMARG_TOKENS_VERSION
			);
		}
	}
	add_action( 'admin_enqueue_scripts', 'zymarg_sngl_register_shared_brand_assets', 1 );
	add_action( 'wp_enqueue_scripts', 'zymarg_sngl_register_shared_brand_assets', 1 );
}
...
if ( ! function_exists( 'zymarg_sngl_spark' ) ) {
	 ...
	 */
	function zymarg_sngl_spark( $args = array() ) {
		$args = wp_parse_args(
			$args,
			array(
				'size'  => 'md',
				'label' => __( 'ZYMARG Discovery Spark', 'zymarg-single-product' ),
				'class' => '',
			)
		);
		...
		// Next preference: the Store Page plugin's wrapper, if that is the only
		// other Spark source active on this site.
		if ( function_exists( 'zymarg_sp_spark' ) ) {
			return zymarg_sp_spark( $args );
		}
